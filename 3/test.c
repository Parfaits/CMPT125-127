...
int main( int argc, char* argv[] )
{
  ...
  /*printf("BEFORE:%i %i\n",min(img, imgwidth, imgheight), max(img, imgwidth, imgheight));
  normalize(img, imgwidth, imgheight);
  printf("AFTER:%i %i\n",min(img, imgwidth, imgheight), max(img, imgwidth, imgheight));*/
  uint8_t *a = half(img, imgwidth, imgheight);
  // uint8_t *a = region_copy(img, imgwidth, imgheight, 0, 0, 100, 200);
  ...
  draw_image_grey( a, imgwidth, imgheight );
